...
static void cetcd_node_parse(cetcd_response_node *node, int is_pre_node, 
            cetcd_response_parse_callback callback, void *userdata) {
    int i, count;
    cetcd_response_node *n;
    if (node) {
        /*
        ...
        printf("\n");
        */
        callback(userdata, node, is_pre_node);
        if (node->nodes) {
            count = cetcd_array_size(node->nodes);
            for (i = 0; i < count; ++i) {
                n = cetcd_array_get(node->nodes, i);
                cetcd_node_parse(n, is_pre_node, callback, userdata);
            }
        }
    }
}

int cetcd_response_parse(cetcd_response *resp, cetcd_response_parse_callback callback, void *userdata) {
    if (resp->err) {
        printf("Error Code:%d\n", resp->err->ecode);
        printf("Error Message:%s\n", resp->err->message);
        printf("Error Cause:%s\n", resp->err->cause);
    ...
    */
    if (resp->node) {
        printf("-------------Node------------\n");
        cetcd_node_parse(resp->node, 0, callback, userdata);
    }
    if (resp->prev_node) {
        printf("-----------prevNode------------\n");
        cetcd_node_parse(resp->prev_node, 1, callback, userdata);
    }
    return 0;
}
