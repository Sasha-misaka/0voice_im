...
//                    User talkUser =  IMContactManager.instance().findContact(lastFromId);
                    ...
                    User talkUser = userMap.get(lastFromId);
                    // 用户已经不存在了
                    if (talkUser != null) {
...
//                        oriContent = Message.getMsgDatainfo(oriContent);
                ...
                User userEntity = userMap.get(peerId);
//                User userEntity = IMContactManager.instance().findContact(peerId); ////不是好友的话。暂时不收录进来,
                if(userEntity!=null) //不是好友的话。暂时不收录进来, 不管是不是好友都收录先
                {
...
//            IMContactManager.instance().reqGetDetaillUsers(noUserIds);
        }
